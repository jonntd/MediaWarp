...
type CacheConfig struct {
	...
	// 播放信息缓存配置
	PlaybackInfo PlaybackCacheConfig `yaml:"playback_info" json:"playback_info"`
}

// PlaybackCacheConfig 播放信息缓存配置
type PlaybackCacheConfig struct {
	// 启用播放信息缓存
	Enabled bool `yaml:"enabled" json:"enabled"`

	// 媒体项信息缓存TTL
	ItemInfoTTL time.Duration `yaml:"item_info_ttl" json:"item_info_ttl"`

	// Strm文件类型缓存TTL
	StrmTypeTTL time.Duration `yaml:"strm_type_ttl" json:"strm_type_ttl"`

	// Alist链接缓存TTL
	AlistLinkTTL time.Duration `yaml:"alist_link_ttl" json:"alist_link_ttl"`

	// 完整播放信息缓存TTL
	PlaybackInfoTTL time.Duration `yaml:"playback_info_ttl" json:"playback_info_ttl"`

	// 预加载热点内容
	EnablePreload bool `yaml:"enable_preload" json:"enable_preload"`

	// 预加载内容数量
	PreloadCount int `yaml:"preload_count" json:"preload_count"`
}
...
func DefaultPerformanceConfig() *PerformanceConfig {
	return &PerformanceConfig{
		Cache: CacheConfig{
			...
			MaxItems:        10000,
			MaxMemoryMB:     100,
			PlaybackInfo: PlaybackCacheConfig{
				Enabled:         true,
				ItemInfoTTL:     30 * time.Minute,
				StrmTypeTTL:     1 * time.Hour,
				AlistLinkTTL:    10 * time.Minute,
				PlaybackInfoTTL: 15 * time.Minute,
				EnablePreload:   false,
				PreloadCount:    100,
			},
		},
...
func (pc *PerformanceConfig) optimizeForProduction() {
	...
	// 优化播放信息缓存
	pc.Cache.PlaybackInfo.Enabled = true
	pc.Cache.PlaybackInfo.ItemInfoTTL = 1 * time.Hour
	pc.Cache.PlaybackInfo.StrmTypeTTL = 2 * time.Hour
	pc.Cache.PlaybackInfo.AlistLinkTTL = 15 * time.Minute
	pc.Cache.PlaybackInfo.PlaybackInfoTTL = 30 * time.Minute
	pc.Cache.PlaybackInfo.EnablePreload = true
	pc.Cache.PlaybackInfo.PreloadCount = 500
